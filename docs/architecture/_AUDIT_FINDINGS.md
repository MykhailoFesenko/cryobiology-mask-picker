...
> Знахідки під час документування/аудиту (трек `AUDIT_AND_DOCS_PLAN.md`). Кожен
...
> верифікацією; **ризиковані/неоднозначні — НЕ вгадувати, спитати юзера** (краще
...
## Статуси
`logged` (записано) · `asked` (спитано юзера, чекаю) · `approved` (юзер ОК фіксити) ·
...
- **Зворотний кейс:** НЕ «фіксити» зміною коду навмання — selections.json як робочий store коректний. Питання лише: чи треба синхронізувати cleanup.json на bake-all/finalize (для точного provenance у ZIP)? → **ризиковане/неоднозначне, спитати юзера** перед будь-яким код-фіксом.
- **Дія:** ✅ fixed (2026-06-01, юзер обрав «синхронізувати на bake»): `data_sync.bake_with_resync` тепер кличе новий `_sync_cleanup_json_after_bake` — пише `cleanup.json[stem].rejected` = фактично запечений rejected, зберігаючи `user`+`markers`, на КОЖЕН bake-шлях (polygons-export/rebake/cleanup-export/bake-all/finalize). Provenance у ZIP завжди свіже. **pytest 236/236.**
...
- **Дія:** ✅ fixed (2026-06-01, рішення юзера: instance+semantic ЗАВЖДИ, group-masks за вибором): `bake_all.py --pack` тепер рендерить `semantic/` **ЗАВЖДИ** + `mask_groups/` **лише за `--group-masks`** (реюз tested `baking.export_derived_masks`). semantic=per-class, mask_groups=per-group-order → **id-value-незалежні** → коректні для dense deliverable, хоч рендеряться з sparse `selected/`. **Verified end-to-end read-only на `data/vesicles_good`:** без прапора 16 semantic / 0 mask_groups; з `--group-masks` 16/16; дані не змінено. pytest 236/236.

### F-009 — `--pack` layout не збігається з тим, що читає `clusterization.py`   🟠 med · logged (рішення юзера)
...
- **Зворотний кейс:** перейменувати `masks_npy→npy` у `--pack` — але це змінює усталений layout
  (можливо інші консюмери/скрипти очікують `masks_npy/`). Тому — рішення юзера, не авто-фікс.
- **Дія:** logged. Опції: (1) `--pack` пише `npy/` замість `masks_npy/`; (2) лишити, замовник перейменовує;
  (3) додати у README deliverable-а мапу імен. Спитати юзера. Також: `mask_groups` png у `--pack` —
...
- **Дія:** ✅ fixed (2026-06-01, юзер авторизував зсув до 50%): backstop-поріг `baking.py` **0.15→0.50** —
...
- **Симптом:** на vesicles_good безпечно (raw_max 6931). Але **публічний репо** — інші юзери на СВОЇХ даних
...
> F-005 (JS BASE guard), F-006 (handoff-константи), F-009 (deliverable layout npy vs masks_npy — рішення юзера),
