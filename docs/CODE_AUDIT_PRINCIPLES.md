...
> Перед публічним релізом пройти весь реєстр §3 по цьому чеклісту.
...
- Front: браузерна верифікація — реальні пікселі canvas / DOM / network, не screenshot-«на
...
## 5. Процедура pre-release аудиту
...
2. Прогони інваріант-верифікатори: `tools/audit_export.py`, `tools/desync_invariants.py`,
   `tools/verify_bug3_clusterization.py`.
3. pytest повний + node --check усіх JS.
4. Браузер-smoke ключових жестів (браузерна верифікація): cleanup toggle, polygon draw/edit/
